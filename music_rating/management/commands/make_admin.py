...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "username",
            type=str,
            help="The username of the user you want to promote to admin",
        )
        parser.add_argument(
            "current_user",
            type=str,
            help="The username of the user running this command",
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if not current_user.is_staff:
                self.stdout.write(
                    self.style.ERROR("You must be an admin to run this command.")
                )
                return
        except User.DoesNotExist:
            self.stdout.write(
                self.style.ERROR(f"User {current_user_username} does not exist.")
            )
            return
